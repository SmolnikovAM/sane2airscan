...
void AvahiPublisher::start() {
  ...
  if (poll_ == nullptr) {
    throw std::runtime_error("avahi_threaded_poll_new failed");
  }

  client_ = avahi_client_new(avahi_threaded_poll_get(poll_), AVAHI_CLIENT_NO_FAIL,
  ...
  if (avahi_threaded_poll_start(poll_) < 0) {
    ...
    avahi_threaded_poll_free(poll_);
    poll_ = nullptr;
    throw std::runtime_error("avahi_threaded_poll_start failed");
  }
...
void AvahiPublisher::client_callback(AvahiClient *client, AvahiClientState state,
  ...
  case AVAHI_CLIENT_FAILURE:
    ...
    avahi_threaded_poll_quit(publisher->poll_);
    break;
...
void AvahiPublisher::create_services(AvahiClient *client) {
  ...
  txt = avahi_string_list_add_pair(txt, "cs", "color,grayscale");
  txt = avahi_string_list_add_pair(txt, "is", "platen");
  txt = avahi_string_list_add_pair(txt, "uuid", config_.uuid.c_str());
  txt = avahi_string_list_add_pair(txt, "representation", "/eSCL/ScannerIcon");
